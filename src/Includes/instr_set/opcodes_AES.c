...
void __bea_callspec__ aesimc(PDISASM pMyDisasm)
{
    ...
    if (GV.OperandSize == 16) {
        ...
        (*pMyDisasm).Instruction.Category = AES_INSTRUCTION;
           ...
           (void) strcpy_s ((*pMyDisasm).Instruction.Mnemonic,16, "aesimc ");
        #endif
        GV.SSE_ = 1;
        GxEx(pMyDisasm);
        GV.SSE_ = 0;
...
void __bea_callspec__ aesenc(PDISASM pMyDisasm)
{
    ...
    if (GV.OperandSize == 16) {
        ...
        (*pMyDisasm).Instruction.Category = AES_INSTRUCTION;
        #ifndef BEA_LIGHT_DISASSEMBLY
           (void) strcpy_s ((*pMyDisasm).Instruction.Mnemonic,16, "aesenc ");
        #endif
        GV.SSE_ = 1;
        GxEx(pMyDisasm);
        GV.SSE_ = 0;
...
void __bea_callspec__ aesenclast(PDISASM pMyDisasm)
{
    ...
    if (GV.OperandSize == 16) {
        ...
        (*pMyDisasm).Instruction.Category = AES_INSTRUCTION;
        #ifndef BEA_LIGHT_DISASSEMBLY
           (void) strcpy_s ((*pMyDisasm).Instruction.Mnemonic,16, "aesenclast ");
        #endif
        GV.SSE_ = 1;
        GxEx(pMyDisasm);
        GV.SSE_ = 0;
...
void __bea_callspec__ aesdec(PDISASM pMyDisasm)
{
    ...
    if (GV.OperandSize == 16) {
        ...
        (*pMyDisasm).Instruction.Category = AES_INSTRUCTION;
        #ifndef BEA_LIGHT_DISASSEMBLY
           (void) strcpy_s ((*pMyDisasm).Instruction.Mnemonic,16, "aesdec ");
        #endif
        GV.SSE_ = 1;
        GxEx(pMyDisasm);
        GV.SSE_ = 0;
...
void __bea_callspec__ aesdeclast(PDISASM pMyDisasm)
{
    ...
    if (GV.OperandSize == 16) {
        ...
        (*pMyDisasm).Instruction.Category = AES_INSTRUCTION;
        #ifndef BEA_LIGHT_DISASSEMBLY
           (void) strcpy_s ((*pMyDisasm).Instruction.Mnemonic,16, "aesdeclast ");
        #endif
        GV.SSE_ = 1;
        GxEx(pMyDisasm);
        GV.SSE_ = 0;
...
void __bea_callspec__ aeskeygen(PDISASM pMyDisasm)
{
    ...
    if (GV.OperandSize == 16) {
        ...
        (*pMyDisasm).Instruction.Category = AES_INSTRUCTION;
        #ifndef BEA_LIGHT_DISASSEMBLY
           (void) strcpy_s ((*pMyDisasm).Instruction.Mnemonic,16, "aeskeygenassist ");
        #endif
        GV.ImmediatSize = 8;
        GV.SSE_ = 1;
        GxEx(pMyDisasm);
